class Solution:
    ...
    def numIslands(self, grid):
        ...
        isVisit = [[0 for j in range(n)] for i in range(m)]
        row_vector, col_vector = [1, 0, -1, 0], [0, 1, 0, -1]

        # Return 1 if there is an island and not visited
        def isNotVisit(x, y):
            if x >= 0 and x < m and y >= 0 and y < n and grid[x][y] == 1 and isVisit[x][y] == 0:
                return 1
            return 0
        ...
        for row in range(m):
            for col in range(n):
                if isNotVisit(row, col):
                    isVisit[row][col] = 1
                    dfs(row, col)
                    # Count += 1 if there is an island and not visited at function `dfs`(means no adjacent)
                    count += 1
        return count
